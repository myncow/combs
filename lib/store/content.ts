  ...
  try {
    ...
    await db.transaction(async (tx) => {
      await tx.insert(siteSettingsTable).values({
        ...
        appName: DEFAULT_SITE_SETTINGS.appName,
        defaultSeoTitle: DEFAULT_SITE_SETTINGS.defaultSeoTitle,
        defaultSeoDescription: DEFAULT_SITE_SETTINGS.defaultSeoDescription,
        metadataTitleTemplate: DEFAULT_SITE_SETTINGS.metadataTitleTemplate,
        openGraphTitle: DEFAULT_SITE_SETTINGS.openGraphTitle,
        openGraphDescription: DEFAULT_SITE_SETTINGS.openGraphDescription,
        footerCopy: DEFAULT_SITE_SETTINGS.footerCopy,
        supportEmail: DEFAULT_SITE_SETTINGS.supportEmail ?? null,
        contactUrl: DEFAULT_SITE_SETTINGS.contactUrl ?? null,
        updatedAt: new Date(),
        publishedAt: null,
      });
      ...
      await ensurePageSeed(tx, DEFAULT_HOME_PAGE, "home");
      await ensurePageSeed(tx, DEFAULT_GALLERY_PAGE, "listing");
      await ensurePageSeed(tx, DEFAULT_LEADERBOARD_PAGE, "listing");
    });
  } catch (error) {
    console.error("[content:ensureEditorialContentSeeded] fallback", error);
  }
}
...
export async function getSiteSettings(): Promise<SiteSettings> {
  ...
  try {
    ...
    const db = getDb();
    const rows = await db.select().from(siteSettingsTable).where(eq(siteSettingsTable.id, DEFAULT_SITE_SETTINGS.id)).limit(1);
    const row = rows[0];
...
export async function getPageByKey(key: "home" | "gallery" | "leaderboard"): Promise<PageContent | null> {
  if (!hasDatabaseUrl()) {
    if (key === "home") return DEFAULT_HOME_PAGE;
    if (key === "gallery") return DEFAULT_GALLERY_PAGE;
